# 진행 현황 핸드오프 이력 (2026-09-23 기준, 54차 갱신)
...
## `2026-09-quick-fixes-batch` 완료 — 소규모 결함 4건 일괄 정리 (49차, 2026-09-10~11)

게이트2 승인 후 `master`에 squash 병합·push 완료 — **`9d89c50`**. 브랜치
`test/2026-09-quick-fixes-batch`(커밋 10개)는 11.1절 관례대로 로컬 보존한다.

**한 사이클에 REQ 4개 / TASK 10개를 묶은 첫 배치 사이클이다.** 프로덕션 변경은 4파일
**+285/−75**뿐이고 나머지 5,400여 줄은 전부 테스트 자산이다. 회귀는 688건 → **710건**
(+22, 감소 0), 77클래스 → **81클래스**. **QA Pass 8건 전부 1회차, 구현 재작업 0회.**

### 구성

| REQ | 내용 | TASK | 커밋 |
|---|---|---|---|
| 001 | 업로드 섹션 잠금 우회 차단(2단 방어) | 001 · **002**(실브라우저 관측) | `82bfcf0` |
| 002 | 관리자 "내 활동" parity | 003(구현 0줄) · 004 | `4a4dad0` |
| 003 | `AnalysisStatistics` 카운터 원자성 | 005 · 006 · 007 | `b3e3616` `c40cf21` `7bfb42e` `074fec0` |
| 004 | `folderPathStr` 미처리 `Path.of()` 예외 | 008(재현) · 009(수정) | `df6efaf` `a84515c` `afe0586` `a4ef870` |
| — | 전체 회귀·마감 | **010**(메인 세션 직접) | (코드 변경 0) |

### ⭐ 이 사이클의 본론 — "단언이 통과했다"는 아무것도 증명하지 않는다

42차 이후 쌓아온 검증 원칙이 이번에 **세 번째 항목까지 채워져 하나의 계열로 정리됐다.**

| 절 | 원칙 |
|---|---|
| §0.4 | 음성 결과("0건")에는 **양성 대조군**을 붙인다 (STRUCTURE.md 20절) |
| **§0.8**(신설) | 대조군이 있는데도 음성이면, **하네스가 실제 코드 경로를 모델링하는지** 먼저 확인한다 |
| **§0.11**(신설) | 기존 케이스의 판정을 바꾸지 않는 변경(심층 방어·리팩터링)은 **"이 변경을 빼면 깨진다"**는 전용 근거를 갖는다 |

셋을 한 줄로 줄이면: **"측정값이 있다"·"단언이 통과했다"는 그 자체로 아무것도 증명하지
않는다 — 반대 상황에서 다르게 나오는 것까지 보여야 근거가 된다.**

두 신설 절 모두 **실패에서 나왔다.** §0.8은 TASK-005에서, §0.11은 TASK-008에서.

### §0.8이 나온 경위 — 하네스가 결함을 비켜갔다

TASK-005는 REQ-003의 두 손실 패턴을 재현하는 TASK였다. P2(read-modify-write)는 5/5 라운드
100% 재현됐는데 **P1(stale write)은 125라운드 전부 손실 0**이었다. dev는 §0.5대로 정상 중단하고
"설계 전제가 실측과 다르다"고 보고했다.

**그런데 원 관측을 다시 읽으니 말이 맞지 않았다.** 이 REQ를 만든 `bug-suspects.md` 항목의 실패
사례가 **`expected: <2> but was: <1>`** 이고 그 대상이 **P1**이었다. 파일 2개짜리에서 최종값이
1로 남은 것이다.

**원인은 하네스 형태였다.** 실제 `runAnalysis()`는 **파일 1개 = 태스크 1개 = 증가→대입 1회**인데
하네스는 스레드당 2,000~20,000회 **루프**였다. 루프에서는 종반 대입이 전부 최댓값 근처라 최종값이
자연히 수렴한다 — **결함이 있어도 0이 나온다.** 실경로를 모델링하자 재현됐다:

```
files=2  10만 시행  손실 2~3회 (0.002~0.003%)  최대손실 1   ← 최종값이 1
files=8  10만 시행  손실 12회  (0.012%)        최대손실 7
```

**dev가 멈춘 판단 자체는 옳았다.** 문제는 결론의 범위였다 — "이 하네스에서 관측되지 않았다"가
"재현되지 않는다"로 넓어졌다. §0.8이 그 경계를 명문화했고, **보고 문구를 "이 하네스에서는
관측되지 않았다"까지로 제한하고 전제 폐기 판단은 PL이 하도록** 나눴다.

**설계 §4.1은 정정하지 않았다** — 손실 매커니즘 서술은 실측으로 지지된다. 정정이 필요했던 것은
"16스레드 하네스에서 관측된다"는 **재현 방법에 대한 기대**였다.

### §0.11이 나온 경위 — 고치지 않아도 초록불인 항목

REQ-004의 수정은 세 갈래였다: A안(`folderPathStr` 검사) + ⑧(a)(`try` 범위 확장) +
⑧(b)(`outputPathStr` 검사). 그런데 **⑧(a)(b)는 둘 다 심층 방어라 R2/R3의 RED/GREEN을
좌우하지 않는다.** R2/R3를 통과시키는 최소 조건은 A안 하나뿐이므로, **⑧(a)(b)를 구현하지 않아도
테스트는 초록불**이고 그 상태로 Pass를 주면 공허한 통과다.

dev가 이를 발견하고 **범위 밖이라 케이스를 추가하지 않고 지적만 남긴 것**이 옳았다. PL이 §0.11로
승격하고 전용 근거를 설계했다:

| 항목 | 전용 근거 | 전이 |
|---|---|---|
| ⑧(b) | **R5**(동작) | C0 RED(805행 `InvalidPathException`) → C1 GREEN(**문구 문자 단위 일치**) |
| ⑧(a) | **S1**(정적) + **R6**(동작) | C0·C1 RED → C2 GREEN (R6는 818행 NPE) |

그리고 **커밋을 3분할**(C0 케이스만 / C1 검사 / C2 try 확장)해 각 시점의 실행 결과를
**3상태 매트릭스 21셀**로 남겼다. 추정 셀 0개다.

### 검증이 세 겹으로 겹친 지점

**S1의 "C1에서 RED"를 세 주체가 각각 확인했다.** dev가 C2에서 S1 검사 로직을 한 번 고쳤기
때문에(C1이 추가한 주석이 메서드명을 언급해 생긴 오탐), **고친 뒤에도 RED인가**가 검증돼야 했다.

- dev — 보강 후 로직으로 C0·C1 재실측
- **메인 세션** — `git worktree`로 C1 소스에 **보강 후** 테스트를 얹어 실행 → RED
- **QA** — 같은 worktree 방식으로 C1 시점의 **미보강** 로직으로 실행 → RED

두 방향이 다 막혔다. QA는 여기에 더해 **메인 세션이 "확인하지 않았다"고 명시한 두 항목**
(R5의 C0 RED, R6의 C0/C1 RED)을 직접 재현했다 — **"확인하지 않은 것을 명시하면 다음 층이
메운다"**가 실제로 작동한 사례다.

### work-order 명세 오류 2건 — 둘 다 구현 재작업 0건

**dev가 조용히 우회하지 않고 PL 판단을 요청해 잡혔다.**

**① TASK-006 setter 미동기화 근거(§0.9)** — dev가 "동기화하면 수정 전 대조군이 무너진다"고 적었다.
메인 세션이 대조 실험으로 반증했다:

```
plain(현재)           P2누적손실 61,576   P1누적backtrack 17,139
synchronized(가정)    P2누적손실 85,421   P1누적backtrack 609
```

P2는 **오히려 커진다** — `set(get()+1)`은 복합 연산이라 개별 접근 동기화로 원자적이 되지 않고, 락이
읽기-쓰기 창을 넓힌다. **결정(setter 무변경)은 유지하되 근거를 교체**했다. QA도 독립적으로 같은
결론에 도달했다.

**② TASK-009 S1 판정식(§0.15)** — v5가 적은 단언이 **Java 문법상 만족 불가능**했다.
`try (Stream<Path> stream = Files.walk(folderPath))`의 `folderPath`는 try보다 앞에서 선언돼야
컴파일되므로(JLS 14.20.3), "그 try 줄 < `Path.of(folderPathStr)` 줄"은 어떤 형태로도 성립하지
않는다. dev가 3형태를 검토해 불성립을 보였고 **PL이 명세만 정정**했다(범위·대안·TASK 개수 불변,
게이트1 재상정 불필요).

**S1 판정 기준 정본**(§0.15.2): `getDashboardStatus` 메서드 범위 안에서, **기존
`catch (Exception …)`이 실제로 달려 있는 try 줄**의 인덱스가 `resolveUserOutputRoot(` /
`resolveProjectOutputRoot(` / `Path.of(folderPathStr)` 각 첫 줄보다 앞일 것.

### REQ-004의 실사용 영향은 현행 배포에서 사실상 없다 (실측)

동일 클래스 파일을 호스트와 배포 컨테이너 양쪽에서 실행해 대조했다. **실제 시나리오 그대로** —
디렉터리는 `myproj`, 요청만 `myproj `:

| | Windows 10 / 17.0.19 | 컨테이너 Ubuntu 26.04 / 17.0.20 |
|---|---|---|
| `new File(p).exists()` | **true**(후행 공백을 잘라 해석) | **false**(다른 파일명) |
| `Path.of(p)` | **InvalidPathException** | **예외 없음** |
| 결과 | 미처리 예외 → 응답 없음 | **`error` JSON 반환 후 정상 종료** |

**Linux에서는 두 겹으로 성립하지 않는다** — 가드가 먼저 막고, 통과해도 파서가 던지지 않는다.
PM이 우선순위를 **"심층 방어 차원의 정리 작업(시급성 낮음)"** 으로 정정했다.

> **다만 구분이 필요하다.** 위 실측은 **후행 공백 케이스**에 대한 것이다. R6가 드러낸 **루트 경로
> NPE**(`Path.of("/").getFileName()`이 null)는 **플랫폼 무관일 가능성**이 있어 ⑧(a)의 방어가 Linux
> 배포에서도 실효가 있을 수 있다 — **추론이며 Linux에서 측정하지 않았다.**

**HTTP 500은 끝내 직접 관측하지 못했다.** 권한 문제가 아니라 **배포 컨테이너에서 결함이 발생하지
않아 관측할 500이 없기 때문**이다. `bug-suspects.md` 항목에 "추론"으로 표기된 채로 둔다 — 실응답을
보려면 Windows에서 앱을 구동해야 하고 게이트1 ⑨가 이미 범위 밖으로 확정했다.

### TASK-010을 메인 세션이 직접 수행했다 (§0.18)

집계·확인 TASK라 구현 판단이 없고, 2,485행 이력을 가로지르는 요약이라 문맥 연속성이 품질 요인이라는
판단이었다. **승인의 핵심 전제는 "TASK-010도 예외 없이 QA 검증을 받는다"** 였고, PL이 **dev
서브에이전트와 동일한 기록 의무**(수행 직후 기록 / 채팅 보고는 완료 불인정 / 근거 행번호 동반 /
수치 출처 구분)를 명문화했다.

**그 검증이 실제로 값을 했다** — QA가 `+22건`을 신규 4클래스의 XML `tests` 속성 합산(6+5+7+4=22)으로
**재검산**했고, `templates/index.html` 무변경 사유를 TASK-001 원본 기록과 대조해 요약이 가공되지
않았는지 확인했다. 미세 격차 1건(S1 정본 문구를 전문이 아니라 취지로 압축)도 QA가 잡았다.

### 메인 세션이 한 오판 1건

**QA가 돌지 않았다고 단정했다.** 결과 파일이 없는 것을 보고 "프로세스가 없다"로 건너뛰었는데,
QA는 실제로 실행 중이었고 정상 완료했다. **음성 결과 하나로 결론짓지 않는다는, 이 사이클이 내내
요구한 원칙을 검증하는 쪽이 어긴 것**이다(46차의 "성공 로그만 세면 실패를 못 본다"와 같은 계열).
그 오판으로 QA를 중복 기동했고(발견 후 중지, 결과 파일 훼손 없음), dev를 PM의 절차 위반과 같은
유형으로 지적한 것도 근거 없는 지적이라 공개 취소했다.

### 운영 절차 개선 3건

- **게이트2 push를 1회로 통합**(48차 사이클에서 첫 push 직후 재잠금돼 문서 push가 막힌 일의 재발
  방지). 갱신 시점을 **"squash 병합 커밋 직후, push 전"** 으로 바꾸고, 승인 요청 시 **push할 커밋
  수를 미리 알린다**. **이번 사이클이 첫 적용이다**(커밋 2건 / push 1회).
- **`bug-suspects.md` 쓰기를 도구 층에서 차단** — `settings.json`의 allow → deny로 이동. QA가 이
  파일을 수정한 일이 있었는데, 확인해 보니 **막혀 있지 않고 오히려 허용돼 있었다.** 지시문만으로
  통제되던 지점이었다.
- **STRUCTURE.md 18.6 신설** — 실브라우저 검증 시 로그인 주소를 `localhost:8803`으로 통일하고, pane
  로그인 상태는 legacy-analyzer 세션이 직접 확인한다. `127.0.0.1`과 `localhost`는 `localStorage`가
  분리되고, Claude 세션마다 pane이 독립이라 다른 세션의 로그인은 오지 않는다.

### 남은 과제

- 42~48차의 남은 과제는 그대로 유효하다.
- **getter/setter 동기화 비대칭** — getter 3개는 `synchronized`, setter 3개는 아니다. 현재 방어선은
  클래스 Javadoc 계약 + `MainApiController` 1파일 한정 정적 감시이고, **setter의 유일한 프로덕션
  호출 경로가 dead code(`updateStatistics()` 호출부 0건)** 라는 사실이 실질적 방어선이다. 그 호출부가
  생기는 순간 비대칭이 실제 위험이 된다.
- **정적 감시의 범위가 1파일 한정** — 산식·호출이 다른 파일로 복사되면 미탐지다(TASK-007·009 공통).
- **S1의 잔여 사각** — 새 `catch (Exception …)`을 신설하는 형태로는 S1을 통과시킬 수 있다.
- **`startAnalysis()`의 미처리 `Path.of()`** — 같은 형태의 노출이 다른 진입점에 남아 있다
  (`bug-suspects.md` 등록됨, 이번 범위 밖).
- **루트 경로 입력을 친절한 문구로 거부하기** — ⑧(a)로 HTTP 500만 면하고 응답 `error`에 예외
  메시지가 그대로 실린다. copy 모드 산식의 `{srcName}`을 루트에서 정의할 수 없다는 **설계 차원
  문제**라는 dev 의견이 함께 넘어갔다.
- **REQ-002 대안 B**(두 화면 렌더러 공유 컴포넌트 통합) — 46차부터 유보 중.
- **`src/test/resources` 고정본이 2개가 됐다**(`pathformula/…before-98b2d15`,
  `counteratomicity/…before-6d9673e`). **`src/` 전역 grep으로 코드 개수를 세면 오판한다** — 범위를
  `src/main/`으로 좁혀야 한다. 47차 착수 시 전제 확인이 정확히 이 grep이었다.

## `2026-09-remaining-ux-fixes` 완료 — 잔여 UX 결함 9건 일괄 정리 (50차, 2026-09-14~16)

게이트2 승인 후 `master`에 squash 병합·push 완료 — **`a6d488f`**. 브랜치
`test/2026-09-remaining-ux-fixes`(커밋 13개, HEAD `3339004`)는 11.1절 관례대로 로컬 보존한다.

**REQ 9개 / TASK 15개(001~013 + 002B·002C)를 한 사이클에 묶었다.** 변경은 25파일
**+4,027/−99** — 프로덕션 11파일은 계열 표(§A.1) 안이고 나머지는 테스트·픽스처·하네스다.
회귀는 착수 81클래스/710건 → **91클래스/771건, 실패 0/에러 0**(+10/+61, 감소 0). **QA 판정
17회 전부 Pass(1회차 15 + 범위 분리 2), 코드 재작업 0, 에스컬레이션 0.** 게이트2 push는 49차에서
정한 대로 **커밋 2건(squash + 이 문서) / push 1회**로 집행했다.

### 구성

| REQ | 내용 | TASK | 브랜치 커밋 |
|---|---|---|---|
| 009 | `startAnalysis()` 진입점 정규화(trim) | 001 | `258c233` |
| 007 | `AnalysisSessionManager.updateStatistics()` dead code 삭제 | 007 | `ee8d62f` |
| 001 | 관리자 분석 이력 상세보고서 PPT 다운로드 | 008(+메인 세션 실브라우저 관측 ADMIN/비-ADMIN) | `7902fc1` |
| 002 | 일시정지 재개 오안내 — `pauseSettled` 상태값 + 프런트 게이팅 | 002 · **002B**(종단 정규화) · **002C**(회귀 차단) · 009 | `223a10f` `a0d2dd1` `db95bc4` `0adee50` |
| 006 | `startAnalysis()` `Path.of()` 미처리 예외 | 003(재현 R4) · 004(조건부 → 실행) | `5380314` `cce9ca2` |
| 004 | 전량실패 PAUSED 완료 패널 카운터 + 전용 안내 | 010(코드 1회차 + 관측 2회차) | `941bc6b` |
| 008 | 루트 경로 조기 거부 + 전용 문구 | 005 | `82fe369` |
| 003 | 추적파일 `.ai-analysis-done.txt` 스캔 제외(단일 출처, 완전일치) | 006 | `a6846c4` |
| 005 | 미설치 로컬모델 선택 차단(A안) | 011(조사, 0줄) · 012(구현) | `3339004` |
| — | 마감 집계 | 013(메인 세션 직접) | (코드 0) |

### ⚠️ 운영 영향 — REQ-005 A안이 만든 의도된 가용성 회귀

`dashboard.js`의 로컬 모델 드롭다운은 이제 **`GET /api/config/llm-models/local-installed`가
`available=false`(Ollama discovery 확인 불가)를 돌려주면 DB 등록 목록을 노출하지 않는다.** 예전에는
"확인 불가" 경고를 붙여 그대로 보여줬다(passthrough). 실환경에서 확인한 동작:

- Ollama 컨테이너가 내려가 있으면(캐시 TTL 15초 경과 후) 드롭다운이
  `로컬 모델 설치 여부 확인 불가` 1개·`disabled`로 바뀌고, 시작 버튼은
  `선택 가능한 로컬 모델이 없습니다…` alert만 띄우며 **서버에 시작 요청을 보내지 않는다.**
- **설치돼 있는 모델(`qwen3:8b`)로도 시작할 수 없다** — 게이트1 ⑤에서 사람이 "선택 자체를 막는다"로
  결정한 A안의 설계상 결과다. Claude(Anthropic) 탭은 영향이 없다.
- 같은 필터가 토글 없는 배포의 `loadAnthropicModelOptions()` 경로에도 적용됐고(A-2), 필터로 목록이
  0건이 되면 `FALLBACK_MODEL_OPTIONS`(Claude 3종 하드코딩)로 떨어지지 않는다 — 떨어지면 못 쓰는
  모델이 선택 가능해지는 새 구멍이 생기기 때문이다.

**운영자에게**: Ollama가 재시작 중이거나 죽어 있으면 사용자는 로컬 provider로 분석을 시작할 수 없다.
이건 결함이 아니라 차단이다. 완화(예: 설치 확인 실패 시 경고만 띄우고 시작 허용)는 정책 변경이므로
게이트2 이관 항목으로 넘어가 있고, 이 사이클에서는 화면을 그대로 기록만 했다.

### 이 사이클의 본론 — 명세 정정 13번, 그중 전제 오류 7건이 전부 "수행자가 멈춰서" 잡혔다

work-order가 **v1 → v14**까지 갔다. 코드 재작업은 0인데 문서 정정이 13번이었다는 것이 이 사이클의
성격이다. PL이 별항 A(설계·명세 전제 오류)로 분류한 7건은 전부 dev 또는 메인 세션이 **"실측이
전제와 다르다"고 멈춰 보고**해서 나왔다.

| # | 정정 | 성격 |
|---|---|---|
| v2 §0.19 | TASK-001 P2 "판정이 바뀌는 조합 수" 2 → 1 | 설계 전제 ≠ 실측 |
| v3 §0.20 | TASK-008 권한 거부 지표 403 → **302**(`/api/admin/**`는 `LoginUrlAuthenticationEntryPoint`, accessDeniedHandler 없음) | 배포 사실 |
| v4 §0.21 | TASK-002 `pauseSettled` 종단 정규화 누락 → TASK-002B 파생 | 설계 누락 |
| v5 §0.23 | TASK-009 DoD 2(d) 인라인 안내 문구가 **원리적으로 관측 불가**(PAUSED 첫 폴링 ≤2초에 패널 숨김) | 관측 요건 |
| v6 §0.26 | TASK-005 루트 가드를 "try 밖"에 두라는 지시가 직전 사이클 정적 계약과 양립 불가 → (c)안 try 첫 문장 | 사이클 간 계약 간섭 |
| v7 §0.29 | TASK-006 DoD 5 예시 `my.ai-analysis-done.txt.bak`는 비지원 확장자라 검증력 0 | 예시의 검증력 |
| **v12 §0.52** | TASK-011 "`qwen2.5-coder:7b` = 미설치 음성 대조군" 전제가 실측(설치돼 있음, 차집합 ∅)과 불일치 | 환경 전제 |

7번째가 특히 중요했다. v11이 "자연 상태 음성 대조군이 있으니 모델 삭제·DB 조작은 전부 금지"라고
지시했는데, 착수 전 실측 3줄(`ollama list` + DB LOCAL 목록)을 돌리자 그 모델이 **이틀 전에 이미
pull돼 있었다**(TASK-010 관측에 쓴 모델이다). 그 상태에서 지시대로 가면 "미설치 모델이 뜨는가"를
관측할 환경 자체가 없었다. PL은 **대안 D(환경 무조작 등가 재현)** 를 채택했다 — A안이 순수 클라이언트
조치라 클라이언트가 "설치 여부"를 아는 유일한 입력은 `local-installed` 응답뿐이므로, **Ollama를
건드리지 않고 그 응답만 스텁**하면 클라이언트 입장에서 완전히 같은 검증이 된다. 브라우저 콘솔에서
`window.fetch`를 그 한 경로에 대해서만 감싸고 프로덕션 함수를 그대로 재실행했으며, 스텁 해제는
새로고침이다(코드·DB·컨테이너 잔존물 0).

**별항 B — 실동작 회귀 1건(v5 §0.22).** TASK-009 실브라우저 관측 중 `/api/my/analysis-history`가
**전량 500**으로 깨졌다. 원인은 이 사이클 밖(`7333ea4`에서 `generateReadme` 컬럼을 추가하면서 엔티티를
primitive로 둔 것, 옛 세션 행 83/94건이 NULL)이었고, 이 사이클의 TASK-002가 그 값을 읽는 경로를 하나
더 열면서 드러났다. ① 잠복 요인은 `loadSessionFromDatabase()`의 예외 삼킴 ② 테스트 픽스처가 재현
대상을 `NOT NULL`로 배제하고 있어 단위·계약 테스트 전량 GREEN이었다 ③ **실브라우저 관측이 그것을
뚫고 잡았다.** TASK-002C(`Boolean` 래퍼 + NULL→기본값 + 픽스처 정정)로 차단했다.

### REQ-005 — "보류"에서 "수정됨"까지

게이트1에서 ⑥(검증 기준 모델)이 보류되면서 TASK-011/012가 막혔고, TASK-013 마감은 "011/012 제외"로
먼저 돌렸다(PL v8 §0.32 — 게이트1 결정 문서 자체가 "013은 ⑥과 무관"이라고 적어 두었다). 다음 날
PL이 확인한 것은 **⑥이 요구한 게 성능 좋은 기본 모델이 아니라 "DB에 등록되고 실제 설치된 모델
1개"였다**는 것 — `qwen3:8b`가 TASK-009 관측에서 이미 선택→시작→성공까지 관측됐으니 확정 가능했다.
그 뒤 011(조사)·012(구현)를 이틀째에 마쳤다.

TASK-011이 밝힌 원인: 2026-09-07의 원 관측("미설치 모델이 뜨고 `local-installed` 호출 기록이 없다")은
**필터 도입(`b515af6`, 같은 날 14:49) 이전 이미지**였다. dev가 도입 직전 커밋(`3cdd9ab`)의
`dashboard.js`를 같은 `node:vm` 하네스로 돌리자 호출 0회 + 2개 표시 — 원 관측과 정확히 일치했고,
현 빌드는 토글마다 실제로 호출한다(서버 로그). **같은 유형의 오진("구버전 컨테이너")이 이 저장소에서
두 번째다**(`2026-09-local-model-mismatch-fix` REQ-001). 관측할 때 배포 이미지가 어느 커밋인지 먼저
적는 것이 이번에도 교훈이다.

(2)(b) "실패 파일이 패치완료로 표시" 오류는 **현행 미재현**이었다 — Ollama를 잠시 멈추고 마커 파일
1개(`[AI 한글 주석 보완 완료]` 문자열, LLM 호출 없이 already 집계) + 실패 2개로 정상 완료 경로에
도달시켰더니 실패 2건이 `처리실패`로 올바르게 렌더됐다. PL이 수정 커밋(`4a7c1d8`, 2026-09-08)을
특정해 "이미 고쳐진 결함의 사후 확인"으로 승격했다. TASK-012는 완료표시 로직을 0줄 건드렸다.

### 재현·검증 방법 중 다음에 그대로 쓸 것

- **`node:vm` 하네스로 `dashboard.js` 전 경로 대조표**(P1~P9 × 조합 ①②③). TASK-012 검증은 이 하네스를
  **수정 없이** 수정 전/후 리비전에 돌려 **셀 단위 diff**를 냈다 — 변화 셀이 정확히 기대한 7개(P2·P2b·P3
  ③, P4·P4b ②③)이고 나머지 24셀 불변임을 dev·QA가 각각 재현했다. 프런트 변경의 회귀 확인은 이 형태가
  정답이다(스크래치 `t011harness/modelSelectPathsHarness.js`, 저장소 밖).
- **전량실패·정상완료 시나리오는 `docker stop legacy-analyzer-ollama`로 만든다** — 모델 삭제 없이
  `Failed to resolve 'ollama'`로 전 파일이 즉시 실패하고, `docker start`로 완전 복구된다. 단 드롭다운은
  정지 **전**에 채워 둬야 하고(정지 후 새로고침하면 A안 차단에 걸려 시작 자체가 막힌다), discovery
  캐시 TTL 15초를 기다려야 `available=false`가 보인다.
- **사용자 일시정지 관측은 파일 수 > 스레드 수여야 PAUSED로 끝난다.** 3파일/스레드 2로 일시정지하면
  남은 태스크가 이미 전부 시작된 상태라 (i-b) 경로로 COMPLETED가 된다(TASK-002B의 의도 동작). 6파일로
  다시 돌려서 PAUSED 정본을 얻었다.
- **재개 버튼 관측**: `resumeAnalysis()`·`logout()`·`pauseAnalysis()`는 네이티브 `confirm()`이라
  자동화에서 막힌다 — `window.confirm=()=>true`로 덮고 alert는 배열로 포집한다. `my-activity.html`은
  fetch 토큰 래퍼가 없어 `Authorization: Bearer`를 직접 붙인다.
- **로그 감시 신호**: `[분석 진행]`은 `session.addRecentLog`(폴링 응답)에만 남고 서버 로그에는 없다.
  서버 로그는 `[API 분석 성공]|[파일 분석 실패]|[병렬 분석 완료]`, DB는 `analysis_history`.

### 실행 환경에서 겪은 일 (다음 담당자가 알아야 할 것)

- **Docker Desktop 엔진이 두 번 무응답**이 됐다(API 500 → `docker ps` 타임아웃, `vmmem` 축소).
  추정 원인은 qwen3:8b 장기 세션 + gradle 회귀 연속 실행으로 호스트(7.8GB, GPU 없음) 포화.
  트레이 **Quit Docker Desktop → 재실행**으로만 복구된다(창 닫기는 backend PID가 그대로라 효과 없음).
  `bug-suspects.md` 469행(대형 로컬모델 메모리 안전장치 부재)의 추가 실측이다.
- **앱 재빌드 = 컨테이너 재생성 = 비-볼륨 경로(`/app/t0xx-src`) 소실.** 이번에 두 번 겪었다. 한 번은
  분석 세션 진행 중에 재빌드해 재개가 0파일을 찾고 이력이 0/0/0으로 덮였고(46차 등록 결함의 재현),
  한 번은 관측 코퍼스 삭제 순서가 PL 지시보다 앞서 나갔다(PL 판정: 위반 아님, 예고된 순서의 실행).
  이후 재빌드는 **로그 전량 덤프 → 롤백 태그(`pre-t012-a6846c4`) → 전상태 스냅샷 → `compose build` +
  `up -d --no-deps app`(`down`/`-v` 금지) → 컨테이너 ID 전후 비교 → 배포본 grep 게이트 → 기동 지표
  → 사후 재확인**의 9단계로 했다. 이 순서를 그대로 쓰면 된다.
- 로컬모델 실측 요지는 `analyzer-plan/docs/chat/dev/2026-09-15-remaining-ux-fixes-local-model-feasibility-measurements.md`에
  정식 전사했다(qwen3:8b: CPU 100%, 약 2.8 tok/s, 300초 read-timeout 상회 다수, 29파일 성공률 약 15%;
  qwen3:4b는 미측정). "이 노트북에서 기본 로컬모델로 부적합"까지가 확정된 결론이고, 더 작은 모델
  재시험은 별도 트랙(노트북 환각 품질 검증 → 서버 PC)으로 사람이 정한다.
- **로그인 요청 절차**(사람 요청): 토큰 만료를 먼저 확인하고, **로그아웃 처리 + `http://localhost:8803/auth/login`
  화면을 띄운 상태로** 어느 계정이 필요한지 요청한다.

### 게이트2로 넘어간 관측 14건 (PM 전결, 이 문서는 목록만)

①~⑨는 v1~v6에서 누적된 것(출력 루트 위치, 302 blob 저장, `pauseSettled=false` 영구 잔존 — `recoverSessions()`가
호출부 0인 죽은 코드라 "재기동까지"가 아니라 영구, 엔티티 primitive+nullable 컬럼 결함군, 예외 삼킴,
인라인 재개 진입점 도달 불가, `MAX_CONCURRENT_CALLS=1`인데 두 번째 스레드가 정확히 300초 타임아웃(재현
2회), `pauseSession()` 종단 미검사 false 고착, 토큰 기반 정적 계약의 감시 공백), ⑩~⑬은 TASK-010 관측에서
(전량실패 응답 `historyId=null`, `[전체 실패] 사유: 알 수 없는 오류`(errorLog 미적재), 전량실패 세션
`analysis_sessions.status=IN_PROGRESS` 잔류, 스레드≥잔여 파일 시 "일시정지됨" 안내 후 COMPLETED), ⑭는
(2)(b) 사후 확인. PM 초안은 ④⑧⑫⑩⑪ 결함 확정·⑬ 의도된 동작이며 **③⑤⑧을 "세션상태 정합성" 한 사이클로
묶는 것이 다음 최우선 후보**로 권고돼 있다. 별도로 `getLocalInstalledModels()` Javadoc이 A-1 이후 사실과
어긋난 것(서버 0줄 지시라 미수정)이 등록됐다.

### 남은 과제

- 42~49차의 남은 과제는 그대로 유효하다(getter/setter 비대칭, 정적 감시 1파일 한정, S1 사각, REQ-002
  대안 B 유보, `src/test/resources` 고정본 grep 오판 주의).
- **A안 가용성 회귀의 처분** — 위 운영 영향 절. 완화 여부는 정책 결정이다.
- **P7/P8(`FALLBACK_MODEL_OPTIONS` 경로)은 시작 시점 가드가 없다** — 이번 사이클 범위 밖으로 확정됐고
  LOCAL 키가 노출되지 않아 REQ-005 대상은 아니지만, 직전 사이클이 "잔여 경로"로 남긴 지점 그대로다.
- **서버측 시작 시점 검증(B안)은 없다.** `ClaudeServiceImpl.setModel()`은 DB 활성 등록만 검사하고 설치
  여부는 보지 않는다 — 클라이언트 차단을 우회하는 직접 API 호출은 막히지 않는다.
- **취소 후 진행 중이던 LLM 요청은 중단되지 않는다** — `cancelAnalysis()` 뒤 Ollama가 수 분간 계속 생성했고
  이력에 실패 1건으로 집계됐다(id102·103). 관측 사실만 있고 등록은 안 했다.

## `2026-09-docs-currency` 완료 — 레거시 문서 6개 정확성 정정 (51차, 2026-09-16)

게이트2 승인 후 `master`에 squash 병합·push 완료 — **`0205868`**. 브랜치
`test/2026-09-docs-currency`(커밋 10개, HEAD `d160338`)는 11.1절 관례대로 로컬 보존한다.

**이 사이클은 문서 전용이다.** `src/**` 변경 0줄, `gradlew build/test` 미실행 — 판정 기준이 "테스트
GREEN"이 아니라 **문서-코드 대조**(실소스 식별자 grep + 양성/음성 대조군 쌍)였다. 변경은 정확히
**6파일 +245/−226**. **QA 10건 전부 Pass, dev 재작업 0건.** 이 문서(handOff.md)는 이번 사이클
수정 금지 대상이었으므로 본문의 낡은 서술은 그대로이고(다음 사이클 이월, 아래 남은 과제), 이 절만
사이클 완료 기록으로 추가한다.
...
| REQ | 내용 | TASK | 파일 | 커밋 |
|---|---|---|---|---|
| 001 | LLM provider 서술 정정 — "설정 하나로 배타 전환"이 아니라 **런타임 리졸버(`LlmClientResolver`)가 DB `llm_model_options`의 provider로 구현체 선택** | 001·003·006·007 | README / ARCHITECTURE / plan / scenario_0 | `3c09f1f` `013a792` `4823b43` `318aeef` |
| 002 | 신규 기능 문서화(세션 제어·failover·PPT 구조 스냅샷 등) + 패키지/클래스 표 보완 | 002·004 | README / ARCHITECTURE | `ac8f0fc` `e05d2f3` |
| 003 | FAQ "PPT는 어떻게 얻나?" 경로 정정(`scripts/pptx/` 성격 명시) | 001 | README | `3c09f1f` |
| 004 | PowerPoint 변환가이드 전면 재작성 — 수동 변환 절차(LibreOffice/Google Slides/Python) 폐기, **서버가 `.pptx`를 자동 생성**하는 9개 절 구성 | 009 | guides/PowerPoint_변환가이드 | `abc5ded` |
| 005 | plan.md P2 서술에 구현 결과·현재 상태(2026-09) 표기 | 006 | plan | `4823b43` |
| 006 | 저작자·유지보수 블록 통일 | 005 | README | `94e4f92` |
| 007 | 프로세스 흐름도 — **mermaid 최초 도입** 2종(분석 세션 시퀀스도, `currentPhase` 상태 전이도) | 005 | README | `94e4f92` |
| — | 가이드 소개 3블록을 재작성 구성에 맞게 정정(v3 추가) | 005 | README | `d160338` |
| — | 교차 스윕 검증(30조합 + 일관성 4쌍, 문서 무수정) | 010 | — | (커밋 0) |

### 이 사이클에서 정한 문서 규칙 (다음 문서 작업에 그대로 적용)

- **새 서술에 소스 라인 번호를 적지 않는다** — 클래스/메서드/엔드포인트/테이블명으로 쓴다. 라인은 곧 낡는다.
- **새 오류 표현 2개를 쓰지 않는다**: ⓐ "`LlmClientResolver`가 `llm.provider` 설정 기반으로 선택"(실제 기준은 DB
  `llm_model_options`의 `provider`), ⓑ "`User` 엔티티의 `anthropicAccessAllowed` 컬럼"(존재하지 않음 — 실제는
  `ANTHROPIC_USER` Role). 분석 문서(`00-analysis-v2.md`)에 이렇게 잘못 적혀 있어 그대로 옮기면 오류가 된다.
- **"낡은 패턴 0건"은 대조군 없이 근거가 아니다** — `git show {시작커밋}:{파일} | grep`으로 수정 전 N>0을 먼저
  보이고, 수정 후 0건, 새 패턴 ≥1건을 나란히 남긴다(STRUCTURE.md §20).
- **낡은 표현 패턴은 두 종류다**(work-order v4 §0.7): 현행 아키텍처를 설명하는 자리에서는 0건 강제, "당시 목표·
  이력·인덱스 소개" 문장은 맥락 판정(원문 보존 지시가 있으면 정상 잔존). 이번에 5건이 정상 잔존으로 확정됐다
  (README 1 / plan.md 2 / scenario_0.md 2).
- **두 문서가 서로 다른 상태 축을 그리면 "전이 집합 완전 일치"를 요구하지 않는다**(v5 §0.9): `ARCHITECTURE.md`
  6-4는 `SessionState.status`(=`AnalysisHistory.status`, 6값) 축, README 상태 전이도는 `SessionState.currentPhase`
  (9값) 축이다. 같게 만들면 코드에 없는 상태를 적게 된다. 판정은 대응 누락/역방향 모순/사실 서술 충돌 3기준.
- **mermaid는 사람이 직접 렌더를 봐야 한다** — 구문이 틀려도 에러 없이 코드블록으로 조용히 깨진다. 이번에는
  브랜치가 push 전이라 GitHub 대신 **mermaid.live**에 코드블록을 붙여 넣어 확인했고("둘 다 그려짐"), 확인 수단·
  결과·코드 전문을 `05-dev-progress.md`에 남겼다. 로컬 HTML 자동 렌더는 브라우저 페인이 로컬 파일을 정적
  스냅샷으로만 열어 불가했다.
- **1 TASK = 1 파일 = 1 커밋** — 같은 파일 체인(README 001→002→005, ARCHITECTURE 003→004)은 순차, 다른 파일은
  선행 QA 판정과 무관하게 진행 가능.

### work-order 정오 5건 — 전부 문면 기안 오류, dev 재작업 0

| # | 정오 | 성격 |
|---|---|---|
| E-1 (v2) | TASK-003 DoD 1 "잔존은 RAG 맥락뿐"이 작업 내용 4번(9절에 과거형 정정 문장 쓰기)과 양립 불가 → §6.3 일반 원칙("잘못된 현재형 서술이 남아 있는가")으로 판정 | DoD 문면 |
| E-2 (v3) | README의 PPT 가이드 소개 3블록이 TASK-009 재작성 후 낡는데 어느 TASK에도 없음 → TASK-005 작업 내용 7번으로 흡수(사람 결정) | 범위 누락 |
| E-3 (v4) | §0.7 "0건 강제"가 원문 보존 지시 문장 5건과 충돌 → 0건 강제/맥락 판정 분리 | 판정 규칙 |
| E-4 (v4) | 설계 FACT-08이 사용자 경로 근거를 관리자 경로까지 일반화(관리자 요약 PPT는 `analysis_` 접두·로그 문구 다름) → 가이드는 경로별 구분 서술 유지, 코드 쪽 불일치는 `bug-suspects.md` 등록 | 설계 범위 누락 |
| E-5 (v5) | TASK-005 DoD 5 "전이 집합 완전 일치" → §0.9 대응 대조 | DoD 문면 |

E-1·E-5는 QA가 판정을 임의 해석하지 않고 "보류 → PL 질의"로 올려 잡혔다(직전 사이클 TASK-006 `.bak`
사례와 같은 처리). E-2는 dev가 범위 밖 관측을 기록에 남겨 사람이 범위에 넣었다.

### 남은 과제 (다음 사이클 후보, 기록만)

- **이 문서(`handOff.md`)의 낡은 서술 이월** — 교차 스윕 결과 낡은 패턴 잔존 P1 6 / P2 2 / P3 2 / P5 16건,
  `docs/advancement/5.completed/scenario_0_completed.md` 3건. 이번 사이클은 게이트1 결정 3번으로 수정 대상에서
  제외했다(이력 문서 성격). 정정한다면 "당시 서술"임을 표기하는 방식이 될 것이다.
- **`scenario_3.md` 롤아웃 계획 재검토 3건**(PM 판단): 1단계 "권한 미부여 = 동작 변화 없음"이 현재 구조
  (`ANTHROPIC_USER` 기본 미부여 → admin 외 Anthropic 선택 불가)에서 불성립 / 3단계 로컬을 사용자별로 좁히는
  수단 없음(활성 LOCAL 모델 등록 = 전체 개방) / 5단계 `LLM_PROVIDER` 게이트 역할 소멸(폴백 기본값뿐). 문서에는
  사실만 병기했다.
- **문서 전체 톤·대상 독자 불통일**(사람 지적) — 범위 밖으로 기록만.
- **코드 쪽**: 관리자 요약 PPT 파일명 `analysis_` 접두·실패 로그 문구가 사용자 경로와 다른 점(`bug-suspects.md`
  `미확인`), 50차의 남은 과제 그대로.

## `2026-09-diagram-readability` 완료 — README mermaid 다이어그램 2종 가독성 개선 (52차, 2026-09-17)

게이트2 승인 후 `master`에 squash 병합·push 완료 — **`a8a7938`**. 브랜치
`test/2026-09-diagram-readability`(커밋 2개, HEAD `7e85466`)는 11.1절 관례대로 로컬 보존한다.

**문서 전용 사이클, `docs/README.md` 단독 +24/−17, `src/**` 0.** 51차에서 신설한 mermaid 다이어그램 2개가
실제 렌더에서 텍스트 겹침·잘림이 있었다(사람이 스크린샷으로 확인 — "그려짐"만 보고 통과시킨 51차의
사각지대). 정보는 그대로 두고 **표현만** 고쳤다. QA 6건 Pass(2건은 기록 누락 보강 후 2회차), 코드 재작업 0.

### 무엇을 바꿨나

| TASK | 대상 | 방법 | 커밋 |
|---|---|---|---|
| 003 | ① 분석 세션 시퀀스도 | participant 2개·메시지 라벨 11개에 `<br/>` 줄바꿈(15개). 48행만 3줄, 나머지 2줄. **`<br/>`·공백을 제거하면 원문과 완전 일치**(단어 삭제 0) | `c452248` |
| 004 | ② 상태 전이도 | `ANALYZING→PAUSED` 등 라벨 4개를 **원문 앞부분만 남기고 뒷부분을 `note right of PAUSED` 5행으로 이동**(3열 대조표로 1:1 추적). 참여자·상태·전이 수 불변 | `7e85466` |

### 이 사이클에서 정한 규칙 (다음 mermaid 작업에 그대로 적용)

- **판정 기준을 올렸다**: 렌더 확인은 "그려짐"이 아니라 **가독성 체크리스트 4항목(C1 최우측 박스 온전 / C2 메시지
  라벨 끝까지 읽힘 / C3 같은 출발·도착 전이 라벨 안 겹침 / C4 박스·선이 글자를 가리지 않음)을 각각 O/X**로.
  뭉뚱그린 "둘 다 잘 보임"은 불통이며, 실제로 한 번 반려됐다.
- **라벨 길이 규칙**(work-order v3 §0.10): 한 줄 = 코드포인트 30자 이하(28 + 오차 2). 예외 E1: 공백·` - `·` + `가 없는
  단일 토큰(URL 경로·식별자)은 토큰 길이까지 허용, 토큰 중간 `<br/>` 금지 / E2: 그 앞 4자 이하 ASCII 접두(GET/POST)는
  같은 줄. 4줄 이상 금지.
- **허용 문법만**: `<br/>`, `note right of X … end note`. `%%{init}%%`는 가독성 미달 시 재작업으로만.
  `autonumber`·`classDef`·`style`·`direction`·`box`·라벨 안 `( ) : | #` 금지, 참여자/상태/전이 추가·삭제·개명 금지.
- **문법 선판정을 먼저 한다**: 새 문법 1개씩 담은 최소 스니펫 4개를 mermaid.live에서 지원/미지원만 확인한 뒤 본 수정.
- **재현 산출물은 관측 세션 안에서 확보한다** — 붙여 넣은 코드가 URL에 인코딩되는 mermaid.live `#pako:` 공유 URL이
  최적이며, 코디네이터가 같은 원문으로 생성한 URL도 인정된다(v2 §0.9 — "누가 만들었나"가 아니라 "누구나 같은 화면을
  재현할 수 있는가"). 산출물 0건("구두 보고만")이 이번에 Fail 1건을 만들었다.
- **D-05(가독성 2회 연속 미달 시 원복)의 "미달"은 C1~C4 판정 결과만** 가리킨다. 기록·형식 누락에 의한 QA Fail은
  산입하지 않는다(v4 §0.11.5 매트릭스: ①가독성 미달 ②산출물 누락 ③기록 누락 ④행번호 미제시 ⑤work-order 모순).
- **정보 무손실 증명**: ①은 `<br/>`·공백 제거 시 원문 완전 일치, ②는 "축약 전 → 축약 후 → 대응 note 행" 3열 표.
  대조 정본은 작업 트리가 아니라 `git show {시작커밋}:docs/README.md`.

### 남은 한계 (TASK-006 §4, 수정하지 않음)

- ② 150행(39자)·153행(63자) 긴 라벨은 목적지가 달라 겹침 원인이 아니라 축약하지 않았다(note 신설은 범위 밖).
- ① 63·72행은 E2 예외(경로 토큰 34·38자) — 뷰포트가 좁으면 여전히 길다.
- `note` 배치는 뷰포트 폭에 따라 PAUSED 오른쪽/아래로 달라진다(결함 아님).
- ② 151행은 판정선 정확히 30자.
- 51·52차 남은 과제(handOff 본문 이월, scenario_3 롤아웃 계획, 톤 통일, 세션상태 정합성)는 그대로.

## `2026-09-doc-currency-v2` 완료 — README mermaid 3종 보강·신설 + 토큰 문서 현행화 (53차, 2026-09-18~22)

게이트2 승인 후 `master`에 squash 병합·push 완료 — **`8ba9e90`**. 브랜치
`test/2026-09-doc-currency-v2`(커밋 6개, HEAD `76fd45f`)는 11.1절 관례대로 로컬 보존한다.

**문서 전용 사이클, `docs/` 3파일 +106/−24, `src/**` 0행.** TASK 10건(001~008 + 신설 009·010) 전부 종결.
QA Pass 9건, Fail 이력 = TASK-005 1회차(C3 X) 1건, D-05 가독성 미달 카운터 1회에서 종결(환원 없음).
work-order는 **v1 → v9**까지 갔고 전부 PL 전제 정정이었다(코드 재작업 0).

### 구성

| REQ | 내용 | TASK | 커밋 |
|---|---|---|---|
| 001 | ① 시퀀스도에 `README 생성 옵션 켬` / `anthropic 모델 + API 키 미설정` opt 2블록 보강, ② `FINALIZING→COMPLETED` 라벨을 "구조 스냅샷 저장 후 README 생성 - 조건부"로 정정 | 002·003 | `42ae520` `76f9b52` |
| 002 | ③ **사용자 화면 흐름(UI 플로우) `flowchart TD` 신설**(43노드·52엣지, 화이트리스트 W1~W3) | 004 | `a20eb33` |
| — | ② `ANALYZING→PAUSED` 3라벨 겹침 해소 — **directive 1줄** `%%{init: {"layout":"dagre", "state": {"nodeSpacing": 40}}}%%`, 라벨 무손실 | 009(신설) | `ddf847e` |
| 003 | 문서-코드 괴리 27건 탐색(Explore, P1 2/P2 9/P3 16) → P1 2건 반영: `technical/TOKEN_EXTRACTION_IMPLEMENTATION.md`·`ANALYSIS_METRICS_DB_SCHEMA.md`의 토큰 누적 서술을 **현재 동작(전역 카운터)** 으로 교체, "관찰된 동작 / 확인 대기" 층 분리 | 006·007 | `93eb7e1` |
| — | README 메타 `마지막 업데이트: 2026-09-22 (…)` + 최종 회귀 | 008 | `76fd45f` |
| — | 문법·렌더 사전검증 / 사람 렌더 확인 C1~C7 2회차 / 배율 원인 관측 | 001·005·010 | (무수정) |

### 본론 — TASK-009가 네 번 정지하며 밝힌 것 (다음 mermaid 작업의 전제)

사람 1회차 C3 X("`ANALYZING→PAUSED` 3라벨이 붙어 보임")를 고치는 데 work-order가 v4→v9까지 5번 바뀌었다.
매번 **"한 층 아래를 보지 않고 전제한 것"** 이 실측으로 부정됐고, dev가 파일을 건드리기 전에 멈춰 보고한 덕에
근거 없는 축약이 들어가지 않았다.

| 차수 | 전제 | 실측 |
|---|---|---|
| v4 접두 축약 | 라벨 폭을 줄이면 겹침이 풀린다 | v12(mermaid.live)에서는 **원래 20px 간격으로 분리돼 있고** 16조합 전부 간격 불변(dagre가 폭+고정 간격 배치). 사람이 본 겹침은 **fit 배율(0.55)에서 붙어 보인 것** |
| v6 fit 배율 판정 | 라벨을 줄이면 배율이 오른다 | **Δ 0.0%** — 배율은 그림의 **세로 높이**에 묶여 있어 라벨 폭은 무관 |
| v7 간격 directive | `state.rankSpacing` 등으로 높이를 줄인다 | v12 기본 엔진이 **ELK**라 `state.*` 간격 키를 **읽지 않음**(`getConfig()`로 전달 확인, 효과 0). v11(dagre)에서만 유효 |
| v8 `layout: dagre` | dagre로 바꾸면 간격 키가 먹는다 | 맞음 — `nodeSpacing` 45 미달 → **40 통과**(`s_fit` 0.55→0.66, +20.3%) → 35 겹침 회귀. 구버전 호환(M-9)은 "v11 M-1=0" 문면이 판별력 0(원래 2건)이라 **"신규 교차 0·면적 순증 없음"** 으로 정정 후 통과 |

부작용: dagre 전환으로 **타 엣지 선이 라벨 4곳 뒤를 지난다**(ELK 0건 → 4건, `nodeSpacing` 무관). 감추지 않고
좌표·확대 크롭으로 위치만 안내해 사람 2회차 C4 판정에 올렸고 **O**로 수용됐다. directive를 지우면 관통은 0이지만
C3가 X로 돌아온다 — 한쪽만 고를 수 있는 구조다.

**확정된 규칙**: 겹침은 기본(fit) 배율 화면좌표로 판정(확대는 보조) / 배율은 긴 축이 지배 / 렌더러(mermaid.live
v12 = ELK)에서 실측한 뒤에만 채택 / 허용 우회는 `layout: "dagre"` + `nodeSpacing`뿐(`fontSize`·`elk.*` 금지) /
사람 관측 산출물은 관측 세션 안에서 파일로(URL·스크린샷·환경 5종). **GitHub 실렌더는 미검증**(push 후 1회 확인).

### 문서 규칙 추가 (v3·v4·v9)

- **G-09**: 레거시 문서에 `bug-suspects.md`/`docs/pipeline/**`/사이클 슬러그 인용 금지 — "2026-09-22 시점 확인 대기"처럼 날짜+상태만.
- 결함 의심과 얽힌 서술은 **층1 관찰된 동작(단정형+근거 식별자) / 층2 열린 질문(확인 대기)** 으로 분리, 판단형 문장("버그다/고쳐야/정상이다") 0건.
- flowchart 노드 라벨: 총 24 코드포인트 + 줄당 추정 폭 ≤120px(한글 14/라틴 6.6/공백 4px) + 최대 3줄, 엣지 라벨 12 이하. **사후 실렌더 검증**(줄 수·SVG 크기·버전·URL) 필수.

### 이월·등록

- REQ-003 이월 25건(P2 9·P3 16) — `analyzer-plan …/2026-09-doc-currency-v2/05-dev-progress.md` 316~348행. **(B) 3건**(`TOKEN_EXTRACTION`·`ANALYSIS_METRICS_DB_SCHEMA`·`scenario_0_completed` — "Claude 단일 provider" 전제 붕괴)은 다음 사이클 최우선 후보.
- `bug-suspects.md` 신규 `미확인` 4건: **B1** 전역 토큰 카운터 세션 혼입 / **B2** 로컬 모드 배포에서 Anthropic 사용 시 비용 0 / **B3** `application.properties` 키 미설정 주석 과소 서술 / **B4** 단가 매핑 haiku 폴백. 상태 확정은 PM.
- `nodeSpacing 40`은 v11에서도 작동(폭 −37.5px, 기존 교차 1건 +86px²·총합 −1.2%) — 포장 없이 기재.
- README 파일 목록 용량 표기 드리프트(P3).

## `2026-09-state-diagram-restructure` 완료 — ② 상태 전이도 라벨 겹침 해소 (54차, 2026-09-23)

게이트2 승인 후 `master`에 squash 병합·push 완료 — **`e4bb208`**. 브랜치
`test/2026-09-state-diagram-restructure`(커밋 3개, HEAD `bab18ac`)는 로컬 보존, 검증용 원격 브랜치
`render-check/…`는 같은 push 창에서 삭제했다.

**`docs/README.md` 단독 +3/−4, `src/**` 0행.** 53차가 directive로 막지 못한 GitHub 라벨 겹침을
**다이어그램 구조를 줄여서** 해소했다. QA 6건 전부 1회차 Pass, 연속 Fail 0, 폴백 사다리(F-1~F-4) 미사용.

### 무엇을 바꿨나

`ANALYZING → PAUSED` 전이 **3개 → 2개**:

| 변경 | 내용 |
|---|---|
| 유지 | `사용자 일시정지`(수동) |
| 교체 | `크레딧 소진` → **`자동 일시정지 2종`** (크레딧 소진 + failover 대상 없음 / 선택된 파일 전부 실패를 한 화살표로 통합) |
| 삭제 | `ANALYZING --> PAUSED : 선택된 파일 전부 실패` 행 |
| 보존 | 삭제된 두 경우의 상세 조건은 `note right of PAUSED`의 진입 2·3에 그대로 있고, 설명 문단에 매핑 문장 1개를 추가해 그림↔note를 잇는다(정보 무손실 3열 대조: 3항목 각각 2곳) |

결과(GitHub 실렌더 mermaid **11.17.2**): 라벨 교차 **2건(871.7 + 382.7 = 1,254.4px²) → 0건**,
mermaid.live(v12)도 0건 유지. 사람 최종 판정 **C3′ O / C4 O**. 152행 directive·note 5행·나머지 16개
전이·①③ 블록은 한 행도 건드리지 않았다.

### 이 사이클이 확립한 것 — 렌더러를 "추정"에서 "측정"으로

53차는 GitHub 버전을 모르는 채 v12 기준으로 고쳤다가 실패했다. 이번에는 먼저 버전을 확정했다.

- **버전 확정(등급 A)**: `window.mermaid`는 ES 모듈이라 전역에 없다. 대신 **iframe이 실제 로드한 번들을
  `fetch()`해** `renderer.draw(e,t,"11.17.2",b)`에서 문자열을 뽑았다. 마크업 지문·행동 지문(교차 2건이
  v11 예측과 일치, v12 예측 0건과 불일치)도 같은 결론.
- **대리 측정**: 로컬 `mermaid-cli 11.17.x`가 GitHub와 **엣지 라벨 좌표 편차 0.00px**. 이후 모든 기하
  측정을 로컬에서 하고 GitHub은 최종 확인만 했다. 예상 배율 0.7913이 실측과 소수 4자리까지 일치.
- **판정 기준 이원화**: R-1 = GitHub(최종 판정) / R-2 = 같은 버전 로컬(기하) / R-3 = mermaid.live(기록·회귀).
  R-3의 배치 차이는 **메이저가 달라 정상**이며 후퇴 사유가 아니다.
- **겹침은 두 좌표 기준으로 잰다** — `g.edgeLabel` 루트 bbox와 `foreignObject` 둘 다 0건일 때만 통과.

### 실측으로 뒤집힌 전제 3건

| 전제 | 실측 |
|---|---|
| 53차의 `layout: "dagre"`가 GitHub 배치를 바꾼다 | **완전 no-op**(11.17.2 기본이 이미 dagre). 실효가 있는 건 `nodeSpacing: 40`뿐 |
| ③ UI 플로우가 화면 밖으로 잘린다 | 잘리지 않는다 — `width:100%`+`max-width`라 **0.39배로 축소**되고 가로 스크롤 0. 진짜 문제는 축소로 인한 판독성(C7-b) |
| 라벨 실효 글자 9.09px | **12.11px** — 예상값이 글꼴 12px을 가정했으나 실제 엣지 라벨은 **16px**. 변경 전 12.66px → 12.11px(−0.55px)가 폭 +57.63px의 대가 |

그 밖에: mermaid note 박스는 **난수 시드 hand-drawn 경로**라 같은 입력도 렌더마다 SVG 바이트가 다르다
(대조는 소스 블록·viewBox·측정 지표로 한다). 라벨은 자기 엣지에서 **최대 74.5px까지 떨어져** 배치된다.
SVG 그리기 순서가 `edgePaths` → `edgeLabels` → `nodes`라 **선이 라벨 박스 뒤로 지나가고 글자를 가리지 않는다**.

### 절차 — push 창을 두 번 연 사이클

GitHub이 최종 판정 렌더러이므로 **작업 중간에 원격이 필요**했다. `test/…` 브랜치 push는 상시 금지이므로,
같은 커밋을 가리키는 **검증 전용 브랜치 `render-check/…`를 1회 push**(P①)하고 판정 후 게이트2 창에서
삭제했다(P②에서 squash·handOff·브랜치 삭제 3건을 한 창에 처리). **`master`는 P②까지 오염되지 않았다.**
이 예외를 일반 관행으로 승격할지는 PM/command 범위로 남겼다.

### 이월·기록

- **이월-1** ③ 과폭 축소 판독성(C7 X, 자연폭 2,594 vs 컨테이너 1,012, 39% 축소) / **이월-2** 패닝 편의성
  (②③ 프레임에 Zoom·Reset·Pan 7버튼 실재). 둘 다 이번 사이클 ③ 0행 유지.
- `02-design-v1` §3.3의 **I1 대조군 수치(교차 1건 490.2px²)가 재현되지 않는다**(실측 0건, I3 값이 섞인 것으로
  보임) — 다음 사이클이 그 표를 재활용할 때 재측정 전 신뢰하지 말 것.
- 절차 사고 1건: QA가 `06-qa-results.md` 절 순서를 바로잡으려 **파일을 전체 재작성**했다(append-only 위반).
  총 행수·해당 절 바이트는 PL이 동일 확인했고 피해 0이라 현상 유지했다. 앞으로 순서가 어긋나면 고치지 말고
  **맨 아래에 "순서 정정 메모"를 append**한다.
